...
int main()
{
    ...
    ArrayList* lista = al_newArrayList();
        ...
        FILE *fEmpleados;
    fEmpleados = fopen("./empleado.txt", "r");
    if(fEmpleados != NULL)
    {
        while (!feof(fEmpleados))
        {
            fscanf(fEmpleados, "%d,%[^,\n],%[^,\n],%f,%d", &aux.id, aux.name, aux.lastName, &aux.salary, &aux.sector);
            p0 = newEmployee(aux.id, aux.name, aux.lastName, aux.salary, aux.sector);
            lista->add(lista,p0);
